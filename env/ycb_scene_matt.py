            ...
            if if_stack:
                ...
                for i in range(len(urdfList)):
                    if i == 0:
                        ...
                        while not spare:
                            spare = True
                            xpos = 0.5 + 0.26 * (random.random() - 0.5) - self._shift[0]
                            ypos = 0.8 * self._blockRandom * (random.random() - 0.5) - self._shift[0]
                ...
                for i in range(4):
                    ...
                    # x_offset = 0.15 * math.cos(i*1.57)#0.26->0.18
                    # y_offset = 0.2 * math.sin(i*1.57)
                    x_offset = 0.25 * math.cos(i*1.57)#0.26->0.18
                    y_offset = 0.25 * math.sin(i*1.57)
                    self.wall.append(p.loadURDF(wall_file, self.table_pos[0] + x_offset, self.table_pos[1] + y_offset,
                                     self.table_pos[2] + 0.3,
                                     orn[0], orn[1], orn[2], orn[3]))
                for i in range(4):
                    # x_offset = 0.15 * math.cos(i*1.57 + 0.785)
                    # y_offset = 0.2 * math.sin(i*1.57 + 0.785)
                    x_offset = 0.25 * math.cos(i*1.57 + 0.785)#0.26->0.18
                    y_offset = 0.25 * math.sin(i*1.57 + 0.785)
                ...
                for i in range(len(urdfList)):
                    xpos = 0.45 - self._shift[0] + 0.17 * (random.random() - 0.5)#0.17
                    ypos = -self._shift[0] + 0.23 * (random.random() - 0.5)#0.23
                    obj_path = '/'.join(urdfList[i].split('/')[:-1]) + '/'
                    self.target_idx = self.obj_path.index(os.path.join(self.root_dir, obj_path))
                    self.placed_objects[self.target_idx] = True
                    self.target_name = urdfList[i].split('/')[-2]
