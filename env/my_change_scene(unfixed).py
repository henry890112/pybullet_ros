...
class SimulatedYCBEnv():
    ...
    def cabinet(self, save=False, init_joints=None, num_object=1, if_stack=True,
        ...
        plane_file = os.path.join(self.root_dir,  'data/objects/floor/model_normalized.urdf')  # _white
        table_file = os.path.join(self.root_dir,  'data/objects/table/models/model_normalized.urdf')
        cabinet_file = os.path.join(self.root_dir,  'data/objects/cabinet/model.urdf', )

       
        # self.obj_path = [plane_file, table_file]
        self.cabinet_obj_path = [plane_file, table_file, cabinet_file]
        ...
        self.cabinet_pos = np.array([ -0.1 - self._shift[0], 0.5 - self._shift[1], -.82 - self._shift[2]])
        self.cabinet_id = p.loadURDF(cabinet_file, [self.cabinet_pos[0], self.cabinet_pos[1], self.cabinet_pos[2]],
                                  [orientation[0], orientation[1], orientation[2], orientation[3]], useFixedBase=True, globalScaling=0.7)       
        ...
        if not self.objects_loaded:
            self.cabinet_objectUids = self.cabinet_cache_objects()
        ...
        self.cabinet_objectUids += [self.plane_id, self.table_id, self.cabinet_id]            #從高處落下我只改_randomly_place_objects
        print(self.cabinet_objectUids)
        # 20221230 henry
        ...
        self.collided_before = False
        self.obj_names, self.obj_poses = self.get_env_info()
        self.init_target_height = self.cabinet_get_target_relative_pose()[2, 3]
        return None  # observation
    ...
    def retract(self, x_translation, y_translation, z_translation):
        """
    ...
    def cabinet_cache_objects(self):
        """
        Load all YCB objects and set up
        """

        cabinet_obj_path = os.path.join(self.root_dir, 'data/objects/')
        objects = self.obj_indexes
        cabinet_obj_path = [cabinet_obj_path + objects[i] for i in self._all_obj]

        self.target_obj_indexes = [self._all_obj.index(idx) for idx in self._target_objs]
        pose = np.zeros([len(cabinet_obj_path), 3])
        pose[:, 0] = -0.5 - np.linspace(0, 8, len(cabinet_obj_path))
        pos, orn = p.getBasePositionAndOrientation(self._panda.pandaUid)
        objects_paths = [p_.strip() + '/' for p_ in cabinet_obj_path]
        objectUids = []
        self.object_heights = []
        self.cabinet_obj_path = objects_paths + self.cabinet_obj_path
        self.placed_object_poses = []
        self.object_scale = []
        ...
        for i, name in enumerate(objects_paths):
            ...
            if self._disable_unnece_collision:
                for other_uid in objectUids:
                    p.setCollisionFilterPair(uid, other_uid, -1, -1, 0)
        self.objects_loaded = True
        self.placed_objects = [False] * len(self.cabinet_obj_path)
        return objectUids

    def cache_reset(self, init_joints, enforce_face_target, num_object=3, if_stack=True):
        """
    ...
    def cabinet_cache_reset(self, init_joints, enforce_face_target, num_object=3, if_stack=True):
        """
        Hack to move the loaded objects around to avoid loading multiple times
        """
        ...
        self.collided = False
        self.collided_before = False
        self.obj_names, self.obj_poses = self.cabinet_get_env_info()
        self.init_target_height = self.cabinet_get_target_relative_pose()[2, 3]
    ...
    def cabinet_get_env_info(self, scene_file=None):
        """
        Return object names and poses of the current scene
        """
        ...
        for idx, uid in enumerate(self.cabinet_objectUids):
            if self.placed_objects[idx] or idx >= len(self.cabinet_objectUids) - 2:
                pos, orn = p.getBasePositionAndOrientation(uid)  # center offset of base
                obj_pose = list(pos) + [orn[3], orn[0], orn[1], orn[2]]
                poses.append(inv_relative_pose(obj_pose, base_pose))
                obj_dir.append('/'.join(self.cabinet_obj_path[idx].split('/')[:-1]).strip())  # .encode("utf-8")
    ...
    def cabinet_get_relative_ef_pose(self):    
        """
        Get all obejct poses with respect to the end effector
        """
        pos, orn = p.getLinkState(self._panda.pandaUid, self._panda.pandaEndEffectorIndex)[4:6]
...
########################################################### 
    ...
    def cabinet_get_target_relative_pose(self, option='base'):
        """
        Get target obejct poses with respect to the different frame.
        """
        if option == 'base':
        ...
        pose = list(pos) + [orn[3], orn[0], orn[1], orn[2]]
        uid = self.cabinet_objectUids[self.target_idx]
        pos, orn = p.getBasePositionAndOrientation(uid)  # to target
        obj_pose = list(pos) + [orn[3], orn[0], orn[1], orn[2]]
        return inv_relative_pose(obj_pose, pose)
    ...
    def get_pcd(self, pcd_id):
        obs, joint_pos, camera_info, pose_info, intrinsic = self._get_observation()
        pcd = o3d.geometry.PointCloud()
        pcd.points = o3d.utility.Vector3dVector(obs[0][:3].T)
        axis_pcd = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.1, origin=[0, 0, 0])

        o3d.visualization.draw_geometries([pcd]+ [axis_pcd])
        o3d.io.write_point_cloud(f"./pcd_data/{pcd_id}.pcd", pcd, write_ascii=True)
        o3d.io.write_point_cloud(f"./pcd_data/{pcd_id}.ply", pcd, write_ascii=True)
